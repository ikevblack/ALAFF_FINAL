...
% Compute the reduction to bidiagonal form.  You need to write BiRed.  See
...
% - To form U_A: 
%   Where are the Householder vectors stored? Answer: Lower triangular
%   section under main diagonal
% - To form V_A:
%   Where and how are the Householder vectors stored? Answer: Upper
%   triangular section above superdiagonal
%   What do you need to do to put them in the right format to pass to
%   FormQ? Answer under the main diagonal
%   Where in V_A does the resulting unitary matrix go?
% Uncomment the lines

 temp_U_A = tril(B, -1);              %grab householders under the main diagonal
 U_A = FormQ(temp_U_A, t);            %process them through FormQ
 V_A = eye( m, m );                   %Identity matrix, relevant later
 temp = triu(B,2)';                   %move householders under the main diagonal
 temp = temp(2:m, 1:m-1);             %omit the last vector
 r = r(1:m-1);                        %omit the last scalar(it was never generated by BiRed anyways)
 Q = FormQ(temp,r);                   %process them through FormQ
 V_A( 2:m, 2:m ) = Q;                 %place into VA
